...
export function BudgetsPage() {
  ...
  const [deleteConfirmBudgetId, setDeleteConfirmBudgetId] = useState<string | null>(null);
  ...
  const saveBudget = async () => {
    ...
    try {
      ...
      handlePermissionDenied(err, setError, form.id ? "Failed to update budget" : "Failed to create budget");
    }
  };

  const confirmDeleteBudget = async () => {
    if (!deleteConfirmBudgetId) return;
    try {
      await api.delete(`/api/budgets/${deleteConfirmBudgetId}`);
      setDeleteConfirmBudgetId(null);
      if (form.id === deleteConfirmBudgetId) setForm(createInitialForm());
      setError(null);
      await queryClient.invalidateQueries({ queryKey: ["budgets"] });
      await queryClient.invalidateQueries({ queryKey: ["dashboard"] });
      await queryClient.invalidateQueries({ queryKey: ["notifications"] });
    } catch (err: any) {
      handlePermissionDenied(err, setError, "Failed to delete budget");
    }
  };
  ...
  const updateForm = <K extends keyof BudgetForm>(key: K, value: BudgetForm[K]) => {
  ...
  const handleAmountChange = (value: string) => {
    ...
    updateForm("amount", normalizedValue);
  };

  const deleteTarget = (budgets.data ?? []).find((item) => item.id === deleteConfirmBudgetId) ?? null;
  ...
  return (
    <>
      <section className="glass-panel budgets-page-shell budgets-sticky-layout">
        <div className="sticky-section-head budgets-sticky-head">
          ...
          <div className="form-grid compact-form-grid panel-enter app-form-strip budgets-form-row budgets-form-row-wide">
            <AppSelect value={form.categoryId} onChange={(value) => updateForm("categoryId", value)} options={categoryOptions} placeholder="Select expense category" />
            <AppSelect value={form.accountId} onChange={(value) => updateForm("accountId", value)} options={accountOptions} placeholder="All accessible accounts" />
            <input placeholder="Budget amount" inputMode="decimal" value={form.amount} onChange={(e) => handleAmountChange(e.target.value)} />
            <AppSelect value={form.alertThresholdPercent} onChange={(value) => updateForm("alertThresholdPercent", value)} options={thresholdOptions} placeholder="Alert at 80%" />
            ...
            {(budgets.data ?? []).length ? (budgets.data ?? []).map((item) => (
              <div key={item.id} className="table-row table-row-transactions transaction-data-row budgets-table-data-row">
                ...
                <div className="row-actions transactions-row-actions compact-row-actions compact-row-actions-inline">
                  ...
                  <button className="button ghost small transaction-action-button danger-button delete-action-button" type="button" onClick={() => { setError(null); setDeleteConfirmBudgetId(item.id); }}>Delete</button>
                </div>
              </div>
      ...
      {deleteConfirmBudgetId ? createPortal(
        <div className="modal-backdrop transaction-modal-backdrop" onClick={() => setDeleteConfirmBudgetId(null)}>
          <div className="modal-card modal-card-structured solid-modal-card transaction-dialog-card transaction-confirm-modal" onClick={(event) => event.stopPropagation()}>
            <div className="panel-head overlay-head">
              <div>
                <h2>Delete Budget</h2>
                <p>Confirm removal of this budget from your records.</p>
              </div>
              <button className="icon-button quiet close-icon-button" type="button" onClick={() => setDeleteConfirmBudgetId(null)} aria-label="Close delete budget modal" />
            </div>
            <div className="delete-confirm-copy">
              <strong>{deleteTarget?.category ?? "Budget"}</strong>
              <p>{deleteTarget ? `${deleteTarget.account} / ${deleteTarget.amount} / ${deleteTarget.alertThresholdPercent}% alert` : "This action cannot be undone."}</p>
            </div>
            {error ? <p className="form-error delete-modal-error">{error}</p> : null}
            <div className="modal-actions delete-modal-actions">
              <button className="button ghost" type="button" onClick={() => setDeleteConfirmBudgetId(null)}>Cancel</button>
              <button className="button primary delete-confirm-button" type="button" onClick={confirmDeleteBudget}>Delete</button>
            </div>
          </div>
        </div>,
        document.body,
